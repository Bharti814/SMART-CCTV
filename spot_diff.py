...
def spot_diff(frame1, frame2, FRAME_WINDOW):
	...
	# beepy.beep(sound=4)
	cv2.imwrite("stolen/"+str(datetime.now().strftime('%Y_%m_%d_%H_%M_%S'))+".jpg", frame1)
	# cv2.imwrite("stolen/"+datetime.now().strftime('%-y-%-m-%-d-%H:%M:%S'), frame1)
	cv2.waitKey(0)
